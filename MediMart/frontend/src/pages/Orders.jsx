...
function Orders() {
  ...
  const fetchOrders = async () => {
    try {
      let res;
      if (user.role === "seller") {
        res = await API.get("/seller/orders");
      } else {
            ...
            <div className="order-header">
              ...
              <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
                {order.status?.toLowerCase() === "delivered" ? (
                  ...
                  <span className="invoice-link-btn disabled" style={{ background: '#f8fafc', color: '#94a3b8', borderColor: '#e2e8f0', cursor: 'not-allowed', opacity: 0.7 }} title="Available after delivery">
                    Download Invoice
                  </span>
                )}
                {user.role === "customer" ? (
                ...
                <div key={index} className="order-item">
                  ...
                  <div className="order-item-info">
                    ...
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                      Sold by: <span style={{ color: 'var(--primary)', fontWeight: 600 }}>{item.sellerName || "MediMart"}</span>
                    </p>
